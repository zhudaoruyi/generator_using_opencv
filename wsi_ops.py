...
class PatchExtractor(object):
    ...
    def extract_positive_patches_from_tumor_region(wsi_image, tumor_gt_mask, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            X = np.random.random_integers(b_x_start, high=b_x_end, size=100)
            Y = np.random.random_integers(b_y_start, high=b_y_end, size=100)
            num = 0
            for x, y in zip(X, Y):
                if int(tumor_gt_mask[y, x]) is 255:
                    patch = wsi_image.read_region((x * mag_factor - np.random.randint(0-mag_factor, mag_factor), y * mag_factor - np.random.randint(0-mag_factor, mag_factor)), 0, (299, 299))
    ...
    def extract_negative_patches_from_normal_wsi(wsi_image, image_open, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            num = 0
            for x, y in zip(X, Y):
                if int(image_open[y, x]) is not 0:
    ...
    def extract_negative_patches_from_tumor_wsi(wsi_image, tumor_gt_mask, image_open, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            num = 0
            for x, y in zip(X, Y):
                if int(image_open[y, x]) is not 0 and int(tumor_gt_mask[y, x]) is not 255:
...
class WSIOps(object):
    ...
    def read_wsi_mask(mask_path, level=def_level):
        try:
            ...
            mask_image = np.array(wsi_mask.read_region((0, 0), wsi_mask.level_count - 1, wsi_mask.level_dimensions[wsi_mask.level_count - 1]))
    ...
    def find_roi_bbox(self, rgb_image):
        ...
        # 开运算，将离散的块，合并成离散的大块(去除离散的小块，保留大块)
    ...
    @staticmethod
    def get_bbox(cont_img, rgb_image=np.empty(1)):
